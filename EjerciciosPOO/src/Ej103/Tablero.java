...
public class Tablero extends Applet implements Runnable {
    ...
    private static final int CENTRARX = 125;
    private static final int CENTRARY = 100;
    private int seg = 0;
    private int min = 0;
    ...
    public void init(){
        ...
        for(int i = 0; i < NUMBOTONES-1; i++)
            imagenes[i] = getImage(getCodeBase(), "Ej103/botones/" + (i+1) + ".gif");
        ...
        for(int i = 0; i < ROWS; i++){
            for(int j = 0; j < COLUMNS; j++)
                if(i*ROWS + j < NUMBOTONES)//el último lugar no tiene imagen = null.
                    lugares[i][j] = new Lugar(j*Lugar.SIZE + CENTRARX, i*Lugar.SIZE + CENTRARY, imagenes[i*ROWS + j], (i*ROWS + j) +1);
        }
        try {
            error = getAudioClip(new URL(getCodeBase(), "Ej103/sonidos/error.wav"));
            acierto = getAudioClip(new URL(getCodeBase(), "Ej103/sonidos/correct.wav"));
            exito = getAudioClip(new URL(getCodeBase(), "Ej103/sonidos/exito.wav"));
    ...
    public boolean mouseDown(Event ev, int x, int y){
        ...
        click = new Point((y- CENTRARY)/Lugar.SIZE, (x- CENTRARX)/Lugar.SIZE);
        if(!mover(click))
            error.play();
        else{
            acierto.play();
    ...
    private void paintTime() {
        noseve.setColor(Color.BLACK);
        if(seg < 10)
            noseve.drawString("Time:    " + min + " : 0"+ seg, 400, 15);
        else
            noseve.drawString("Time:    " + min + " : "+ seg, 400, 15);
    }
